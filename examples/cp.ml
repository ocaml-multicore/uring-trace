...
(* (\* let uring_seq_rw_cp ring in_fd pos out_fd buf filesize = *\) *)
(* (\*   let ( >>= ) = link ring in *\) *)
...
(* let uring_seq_cp filepath bufsz = *)
(*   let ring = Uring.create ~queue_depth:1 () in *)
(*   with_file_to_copy filepath (fun in_fd out_fd stat -> *)
(*       let buf = Cstruct.create bufsz in *)
(*       let ( >>= ) = link ring in *)
(*       let filesize = Optint.Int63.of_int stat.st_size in *)
(*       let rec aux file_offset = *)
(*         if Optint.Int63.compare file_offset filesize >= 0 then () *)
(*         else *)
(*           Uring.read ring ~file_offset in_fd buf () >>= fun res _data -> *)
...
(*           aux (Optint.Int63.add file_offset (Optint.Int63.of_int res)) *)
(*       in *)
(*       aux Optint.Int63.zero) *)

(* (\* Optimization 1: batch your reads and writes with vectored read writes *\) *)

(* let take n l = *)
(*   assert (List.length l >= n); *)
(*   let rec aux n = function *)
(*     | [] -> [] *)
(*     | h :: t -> if n > 0 then h :: aux (n - 1) t else [] *)
(*   in *)
(*   aux n l *)

(* let uring_vec_cp filepath bufsz = *)
(*   let queue_depth = 1 in *)
(*   let ring = Uring.create ~queue_depth () in *)
(*   let iovec = List.init queue_depth (fun _ -> Cstruct.create bufsz) in *)
(*   with_file_to_copy filepath (fun in_fd out_fd stat -> *)
(*       let ( >>= ) = link ring in *)
(*       let filesize = stat.st_size in *)
(*       let rec aux offset = *)
(*         let len = filesize - offset in *)
(*         if len = 0 then () *)
(*         else *)
(*           let iovec' = *)
(*             if len >= Cstruct.lenv iovec then iovec *)
(*             else *)
(*               let blocks, rem = (len / bufsz, len mod bufsz) in *)
(*               let iovec_left = take blocks iovec in *)
(*               if rem <= 0 then iovec_left else Cstruct.create rem :: iovec_left *)
(*           in *)
(*           (\* readv/writev loop *\) *)
(*           let offset' = Optint.Int63.of_int offset in *)
(*           Uring.readv ring ~file_offset:offset' in_fd iovec' () *)
(*           >>= fun bytes_read _ -> *)
(*           Uring.writev ring ~file_offset:offset' out_fd iovec' () *)
(*           >>= fun bytes_write _ -> *)
(*           assert (bytes_read = bytes_write); *)
(*           aux (offset + bytes_write) *)
(*       in *)
(*       aux 0) *)

(* (\* Optimization 1: batch your reads and writes without vectored? *\) *)

(* let get_sqe = *)
(*   Uring.sqe_ready *)

(* let uring_cp_fast filepath bufsz = *)
...
(* (\* let uring_vec_cp filepath bufsz vec_rw = *\) *)
(* (\*   let queue_depth = 1 in *\) *)
(* (\*   let ring = Uring.create ~queue_depth () in *\) *)
(* (\*   let iovec = List.init queue_depth (fun _ -> Cstruct.create bufsz) in *\) *)
(* (\*   with_file_to_copy filepath (fun in_fd out_fd stat -> *\) *)
(* (\*       vec_rw ring in_fd out_fd iovec stat.st_size bufsz) *\) *)


(* (\* Optimisation 2: Don't need to wait for writes, can initiate next read/write *\) *)

(* (\* Optimisation 3: SQpoll *\) *)

(* (\* Optimisation 4: Completion polling *\) *)

(* (\* Optimisation 5: Registered files *\) *)
...
(*     | "uring_vec" -> uring_vec_cp filepath bufsz vec_cp *)
