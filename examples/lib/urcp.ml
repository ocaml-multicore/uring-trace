...
let run fixed polling_timeout block_size queue_depth infile outfile () =
  let fn = if fixed then Urcp_fixed_lib.run_cp else Urcp_lib.run_cp in
  fn polling_timeout block_size queue_depth infile outfile ()
...
let cmd =
  ...
  let polling_timeout =
    let doc = "Timeout in ms for Submission queue polling" in
      Arg.(value & opt (some int) None & info ["polling-timeout"] ~docv:"MILLISECONDS" ~doc) in
  let block_size =
  ...
  let info = Cmd.info "urcp" ~version:"1.0.0" ~doc ~man in
    Cmd.v info Term.(const run $ fixed $ polling_timeout $ block_size $ queue_depth $ infile $ outfile $ setup_log)
