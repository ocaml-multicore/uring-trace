...
let cmd =
  ...
  let desc_blk = [ `S "DESCRIPTION"; `P "Low-impact tracing of \
                                    io-uring. This tool generates a \
                                    fuchsia trace file that is meant \
                                    to be displayed on Perfetto. It \
                                    works by attaching onto io-uring \
                                    tracepoints using eBPF \
                                    technology. As a result, it \
                                    requires root priviledges to run."] in
  let usage_blk = [ `S "USAGE"; `P "To use this tool against a \
                                    program, open a separate terminal \
                                    and run $(b, sudo uring-trace) which \
                                    will start the tracing process, \
                                    now execute your program and the \
                                    tool will pickup on newly setup \
                                    rings. You can stop tracing at any \
                                    time by hitting Ctrl-C"] in
